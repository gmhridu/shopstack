...
export const useProductFilters = () => {
  ...
  const filterProducts = useMemo(() => {
    ...
    // Rating
    if (filters.rating) {
      results = results.filter((p) => p.rating.average >= filters.rating!);
    }
    ...
    switch (filters.sort) {
      case "price-asc":
        results.sort((a, b) => a.price.current - b.price.current);
        break;

      case "price-desc":
        results.sort((a, b) => b.price.current - a.price.current);
        break;
